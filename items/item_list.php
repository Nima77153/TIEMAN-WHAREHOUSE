...
function getDynamicImagePath($image_file) {
    ...
    // ==========================================
    // CLOUDINARY / FULL WEB URL
    // ==========================================
    // If database already contains a Cloudinary URL,
    // return it directly.
...
if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($fileTmpPath, $dest_path)) {
        ...
        $stmt = $conn->prepare(
            "UPDATE items SET image = ? WHERE item_code = ?"
        );
...
if (
    isset($_POST['action']) &&
    $_POST['action'] === 'update_stock_dates'
) {
    ...
    if (!empty($dates) && is_array($dates)) {
        ...
        $stmt_update = $conn->prepare(
            "UPDATE items SET stock_date = ? WHERE id = ?"
        );
        ...
        echo json_encode([
            'status' => 'success',
            'message' => 'Stock dates updated automatically!'
        ]);
...
if (isset($_POST['export_excel_action'])) {
    ...
    $selected_ids = isset($_POST['selected_items'])
        ? $_POST['selected_items']
        : [];

    $search = isset($_POST['export_search'])
        ? trim($_POST['export_search'])
        : "";

    $search_param = "%" . $search . "%";

    $category_filter = isset($_POST['export_category'])
        ? trim($_POST['export_category'])
        : "";

    $sort_option = isset($_POST['export_sort'])
        ? trim($_POST['export_sort'])
        : "first_last";
    ...
    header(
        "Content-Disposition: attachment; filename=Warehouse_Report_" .
        date('Ymd_His') .
        ".xls"
    );
    ...
    while ($row = $export_result->fetch_assoc()) {
        ...
        if (!empty($web_image_path)) {
            ...
            echo '<img src="' .
                htmlspecialchars($web_image_path) .
                '" width="55"
                height="55"
                style="display:block;"
                alt="Item">';
...
switch ($sort_option) {
    ...
    case 'a_z':
        $order_query =
            "ORDER BY item_name ASC, description ASC";
        break;

    case '1_z':
        $order_query =
            "ORDER BY LENGTH(item_code) ASC, item_code ASC";
        break;
...
<head>
    ...
    <style>
        ...
        .zoom-popup-view {
            ...
            position:absolute;
            top:50%;
            left:calc(100% + var(--shift-x, 20px));
        ...
        .img-zoom-container:hover .zoom-popup-view {
            display:block;
        }
...
<body>
        ...
        <div class="card-box">
            ...
            >
                ...
                <input
                    type="hidden"
                    name="export_sort"
                    value="<?= htmlspecialchars($sort_option) ?>"
                >
                ...
                >
                    ...
                    <h3>
                        Item Log Catalog View
                    </h3>

                    <div class="d-flex gap-2">

                        <a
                            href="../barcode/print_barcode.php"
                            target="_blank"
                ...
                <div class="row g-2 align-items-center mb-4">
                    ...
                    <div class="col-md-3">
                        ...
                        <input
                            ...
                            id="ui_search"
                            class="form-control"
                            placeholder="Search Part No / Desc / Location..."
                            value="<?= htmlspecialchars($search) ?>"
                        >
                    ...
                    <div class="col-md-3">
                        ...
                        >
                            ...
                            <option
                                value="Store Tieman"
                                <?= ($category_filter == 'Store Tieman') ? 'selected' : '' ?>
                            >
                                Store Tieman
                            </option>
                            ...
                            <option
                                value="Extrusion"
                                <?= ($category_filter == 'Extrusion') ? 'selected' : '' ?>
                            >
                                Extrusion
                            </option>
                            ...
                            <option
                                value="General"
                                <?= ($category_filter == 'General') ? 'selected' : '' ?>
                            >
                                General
                            </option>
                            ...
                            <option
                                value="Civacon"
                                <?= ($category_filter == 'Civacon') ? 'selected' : '' ?>
                            >
                                Civacon
                            </option>
                            ...
                            <option
                                value="Pneumatic"
                                <?= ($category_filter == 'Pneumatic') ? 'selected' : '' ?>
                            >
                                Pneumatic
                            </option>

                            <option
                                value="Lower Chassis Parts"
                                <?= ($category_filter == 'Lower Chassis Parts') ? 'selected' : '' ?>
                            >
                                Lower Chassis Parts
                            </option>

                            <option
                                value="Air Brake Parts"
                                <?= ($category_filter == 'Air Brake Parts') ? 'selected' : '' ?>
                            >
                                Air Brake Parts
                            </option>
                            ...
                            <option
                                value="Other items"
                                <?= ($category_filter == 'Other items') ? 'selected' : '' ?>
                            >
                                Other items
                            </option>

                            <option
                                value="Valve & Pipe Parts"
                                <?= ($category_filter == 'Valve & Pipe Parts') ? 'selected' : '' ?>
                            >
                                Valve & Pipe Parts
                            </option>
                            ...
                            <option
                                value="Liquip Parts"
                                <?= ($category_filter == 'Liquip Parts') ? 'selected' : '' ?>
                            >
                                Liquip Parts
                            </option>
                            ...
                            <option
                                ...
                                <?= ($category_filter == 'Electrical Parts') ? 'selected' : '' ?>
                            >
                                Electrical Parts
                            </option>

                            <option
                                value="Lamp and fitting parts"
                                <?= ($category_filter == 'Lamp and fitting parts') ? 'selected' : '' ?>
                            >
                                Lamp and fitting parts
                            </option>

                            <option
                                value="Malayisa items"
                                <?= ($category_filter == 'Malayisa items') ? 'selected' : '' ?>
                            >
                                Malayisa items
                            </option>
                            ...
                            <option
                                value="China items"
                                <?= ($category_filter == 'China items') ? 'selected' : '' ?>
                            >
                                China items
                            </option>
                    ...
                    <div class="col-md-3">
                        ...
                        >
                            ...
                            <option
                                value="first_last"
                                <?= ($sort_option == 'first_last') ? 'selected' : '' ?>
                            >
                                🔃 Sort: Newest First
                            </option>
                ...
                >
                    ...
                    <button
                        type="submit"
                        name="batch_delete"
                        onclick="return confirm('Are you sure you want to completely remove the selected item assets? This cannot be undone.');"
                ...
                <div class="table-responsive">
                    ...
                    <table
                        id="inventoryTable"
                        class="table table-bordered table-hover align-middle m-0"
                    >
                        ...
                        <thead>
                            ...
                            <tr class="excel-header">
                                ...
                                <th
                                    width="150"
                                    class="date-context-trigger"
                                    style="cursor:context-menu;"
                                    title="Right-click here to set month/year for all rows"
                                >
                                    STOCK DATE
                                </th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="fw-bold text-secondary">
                                    ...
                                    <a
                                        href="../barcode/print_barcode.php?code=<?= urlencode($row['item_code']) ?>"
                                        target="_blank"
                                ...
                                >
                                    ...
                                    <span
                                        class="badge bg-secondary px-2.5 py-1.5 text-uppercase"
                                    >
                                        <?= htmlspecialchars(
                                            $row['category']
                                            ?? 'General'
                                ...
                                <td
                                    class="text-center date-context-trigger"
                                    style="cursor:context-menu;"
                                >

                                    <input
                                        type="date"
                                        class="form-control form-control-sm stock-date-input"
                                        data-id="<?= $row['id'] ?>"
                                        value="<?= $formatted_stock_date ?>"
                                        onchange="autoSaveSingleDate(this)"
                                    >
    ...
    >
        ...
        <button
            ...
            onclick="applyAndAutoSaveBulkMonthYear()"
        >
            Apply to All Rows
        </button>
    ...
    <script>
        ...
        const standardCheckboxes =
            document.querySelectorAll('.row-select-checkbox');

        const batchDeleteBar =
            document.getElementById('batchDeleteBar');
            ...
            .then(
                data => {
                    ...
                    if (
                        ...
                        console.error(
                            "Failed to auto-save stock date."
                        );
                    }
                }
            )
                ...
                el => {
                    ...
                    el.addEventListener(
                        ...
                        function(e) {
                            ...
                            dateContextMenu.style.display =
                                'block';

                            dateContextMenu.style.left =
                                e.pageX + 'px';

                            dateContextMenu.style.top =
                                e.pageY + 'px';
                        }
                    );
                }
        ...
        function applyAndAutoSaveBulkMonthYear() {
            ...
            dateInputs.forEach(
                input => {
                    ...
                    formData.append(
                        `dates[${input.getAttribute('data-id')}]`,
                        newFullDate
                    );
                }
            );
